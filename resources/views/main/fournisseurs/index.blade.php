...
<form action="{{route('fournisseur.index')}}"  method="GET">
    <input value="" type="text" name="search" id="searchInput" placeholder="Recherche par Entreprise" style="background: #f3f3f3 0 0 no-repeat padding-box;
  border: 0;outline:none;border-radius: 6px;box-shadow: 0 3px 6px rgb(54, 54, 54);height: 48px;padding: 10px 10px 10px 50px;" class=" col-9 col-md-3" >
...
@endif --}}
  ...
  <table class='table table-striped'>
    ...
    @foreach ($fournisseurs as $f)
        ...
        <td class="fr">{{$f->nom_entreprise}}</td>
        <td>{{$f->fj}}</td>
        <td>{{$f->adresse}}</td>
        <td>{{$f->telephone}}</td>
        @if ($f->photo=='' || $f->photo==null)
            <td><img src='{{asset('imgs/facts/alt.jpg')}}' ></td>
        @else
